defmodule ZcashAddress.UnifiedAddress do
  ...
  defp parse_receivers(rest, acc) do
    with\
      rest_size when rest_size > 2 <- byte_size(rest),
  ...
  def parse(unified_addr_str) do
    with\
      {:ok, {hrp, decoded, :bech32m}} when hrp in ["u", "utest"] <- Bech32.decode(unified_addr_str),
      ...
      true <- Map.has_key?(map, :sapling) or Map.has_key?(map, :orchard)
    do
      {:ok, hrp, map}
    else
      _ -> :error
